...
{
    public class AutoMapperRealEstateCatalogType : Profile
    {
        public AutoMapperRealEstateCatalogType()
        {
            CreateMap<CatalogType, CatalogTypeDto>().ForMember(d => d.Id, opt => opt.MapFrom(s => s.Id))
            ...
            CreateMap<CatalogTypeDto, CatalogType>().ForMember(d => d.Name, opt => opt.MapFrom(s => s.Name))
                                                      .ForMember(d => d.CreatedUser, opt => opt.MapFrom(s => s.CreatedUser))
                                                      .ForMember(d => d.ModifiedUser, opt => opt.MapFrom(s => s.ModifiedUser));
        }
    }
