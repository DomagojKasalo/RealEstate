...
export class GenericRestService {
  ...
  get url(): string {
    const { resourceName } = this.httpConfig;
    return `${environment.baseAPI}/${resourceName}`;
  }
   ...
   */
  listPagination<T>(options?: HttpGetListOptions): Observable<PaginationResult<T>> {
    const method: HttpMethod = 'GET';
    const url = resolveUrl(this.url, options);
    const requestOptions = extractRequestOptions(options);

    return this.http
      .request<PaginationResult<T>>(method, url, requestOptions)
